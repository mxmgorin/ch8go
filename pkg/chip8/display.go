...
var (
	Chip8DisplaySize = Size{Width: 64, Height: 32}
	SChipDisplaySize = Size{Width: 128, Height: 64}
	lowresScale      = SChipDisplaySize.Width / Chip8DisplaySize.Width
)
...
func NewDisplay() Display {
	...
	d.opRes(false)
	d.planeMask = 1
	d.size = SChipDisplaySize

	for i := range d.Planes {
		d.Planes[i] = make([]byte, d.size.Area())
		clear(d.Planes[i])
	}
...
func (d *Display) togglePixel(plane, x, y int, wrap bool) bool {
	if wrap {
		...
		if x < 0 || x >= d.size.Width || y < 0 || y >= d.size.Height {
...
func (d *Display) spriteFullyOffscreen(x, y, w, h int) bool {
	return x+w <= 0 || x >= d.size.Width || y+h <= 0 || y >= d.size.Height
}
